...
function quizResult()
{
    ...
    auth.onAuthStateChanged(user => {
            ...
            fs.collection(result).doc('_' + id).set({         
                id: '_' + id,
                quizParentTitle,
                quizSubTitle,
                totalQuestion,
...
var totalQuestion = quiz.length, quizSubTitle = subTitle.innerHTML, quizParentTitle = parentTitle.innerHTML;
let id = ctr += 1;
document.getElementById("insertt").onclick = function(){
	Ready();
	    
	firebase.database().ref('results-eb1ca/'+idd).set({
				id: '_' + id,
                quizParentTitle: quizParentTitle,
